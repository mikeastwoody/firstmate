...
# Setup
You are in a disposable git worktree of comms, at a detached HEAD supplied by the worktree pool.
1. **Check your base before starting work.** This brief could not resolve `comms` to a local checkout, so resolve the base yourself first. This check is mandatory: never skip, soften, or postpone it.
...
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/fm-base-e2e2.gPCF6i/home/state/demo-unknown-scout.status'`
...
Firstmate steers you through durable message files in '/tmp/fm-base-e2e2.gPCF6i/home/state/demo-unknown-scout.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/fm-base-e2e2.gPCF6i/home/state/demo-unknown-scout.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-base-e2e2.gPCF6i/home/state/demo-unknown-scout.inbox'/NNN.msg '/tmp/fm-base-e2e2.gPCF6i/home/state/demo-unknown-scout.inbox'/handled/`.
The move IS the acknowledgement: without it firstmate rings again and eventually treats you as stuck. An empty or absent inbox needs no action.

# Definition of done
Write your findings to `/tmp/fm-base-e2e2.gPCF6i/home/data/demo-unknown-scout/report.md`.
